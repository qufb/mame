...
/******************************************************************************
    ...
    See storyware.h for how to setup pages. When loading a cart in the command line,
    the cart's working directory is used to parse the page image files, which should be placed
    alongside the rom image.

    To accomodate cases where there is no artwork available (e.g. BIOS' cart error screen),
    the driver will default to a single screen layout view. The player can activate the 'Pen Target'
    input port to switch between mapping pen coordinates to the tablet or the Storyware.
    ...
    Toggling "Memory Cache" allows the player to observe the differences between test failure and success.
...
#define PAGE_SIZE 0x02500000  // Also used for offsets in 'sega_beena_cart.xml'
#define SCREEN_W 704
#define SCREEN_H 480
...
private:
	...
	required_device<storyware_device> m_storyware;
	...
	// Follows view order specified in "beena.lay"
	enum layout_view : uint8_t
	{
		TABLET_ONLY = 0,
		STORYWARE_ONLY = 1,
		DUAL_UNDER_OVER = 2,
...
*/
TIMER_DEVICE_CALLBACK_MEMBER(sega_beena_state::scanline)
{
	update_view();
...
void sega_beena_state::update_view() {
	const bool has_pages = m_storyware->booklet()->has_pages();
	const bool is_video_connected = m_io_video_config->read() == 0;
	const uint8_t view = has_pages ? (is_video_connected ? DUAL_UNDER_OVER : STORYWARE_ONLY) : TABLET_ONLY;
	if (view != m_configured_view) {
		m_configured_view = view;
		machine().render().first_target()->set_view(m_configured_view);
	}
}
...
void sega_beena_state::irq_wait_speedup()
{
...
void sega_beena_state::beena_arm7_map(address_map &map)
{
	...
	// FIXME: Need to confirm upper bounds / mirrored ranges on hardware
	// Video (registers, palette, sprite data...)
	map(0x40000000, 0x400000ff).rw(FUNC(sega_beena_state::video_reg_r), FUNC(sega_beena_state::video_reg_w)).mirror(0xff00);
	map(0x40010000, 0x400103ff).ram().share("tilemap_sprites");
	map(0x40010400, 0x4001ffff).ram();
	map(0x40020000, 0x400201ff).w(FUNC(sega_beena_state::pal_w)).share("paletteram");
	...
	map(0x50020000, 0x5002007f).r(FUNC(sega_beena_state::io_sensors_r)).share("io_sensor_regs").mirror(0xff80);
	...
	// Realtime clock
	map(0x60000000, 0x600000ff).rw(FUNC(sega_beena_state::rtc_r), FUNC(sega_beena_state::rtc_w)).mirror(0xff00);
	map(0x60010000, 0x6003ffff).ram(); // Unknown

	// MIDI
	map(0x70000000, 0x7000001f).rw(FUNC(sega_beena_state::midi_reg_r), FUNC(sega_beena_state::midi_reg_w)).mirror(0xffe0);
	...
	map(0xc0100000, 0xc017ffff).ram().rw(FUNC(sega_beena_state::tiles_layers_r), FUNC(sega_beena_state::tiles_layers_w));
	map(0xc0180000, 0xc01fffff).ram().rw(FUNC(sega_beena_state::tiles_sprites_r), FUNC(sega_beena_state::tiles_sprites_w));
}

uint32_t sega_beena_state::io_sensors_r(offs_t offset)
{
	uint16_t io_page = 0;
	int16_t io_pen_x = 0;
	int16_t io_pen_y = 0;
	...
	switch (offset) {
		...
		case 0x10/4:
			...
			is_tablet = m_configured_view == DUAL_UNDER_OVER
				? io_pen_y > io_pen_y_max / 2
				: (m_configured_view == TABLET_ONLY && m_pen_target == 0);
			if (is_tablet) {
				if (m_configured_view == DUAL_UNDER_OVER) {
					pen_y_min = io_pen_y_max / 2;
					pen_y_max = io_pen_y_max;
				}
				...
				if (m_configured_view == DUAL_UNDER_OVER) {
					pen_y_min = io_pen_y_min;
					pen_y_max = io_pen_y_max / 2;
				}
		...
		case 0x2c/4:
			...
			if (m_storyware->booklet()->has_pages()) {
				m_storyware->update_effective_page(m_effective_page);
			}
...
uint32_t sega_beena_state::video_reg_r(offs_t offset)
{
	return m_video_regs[offset];
}

void sega_beena_state::video_reg_w(offs_t offset, uint32_t data, uint32_t mem_mask)
{
...
uint32_t sega_beena_state::midi_reg_r(offs_t offset)
{
...
void sega_beena_state::midi_reg_w(offs_t offset, uint32_t data, uint32_t mem_mask)
{
...
void sega_beena_state::device_post_load()
{
...
DEVICE_IMAGE_LOAD_MEMBER(sega_beena_state::cart_load)
{
	...
	if (!image.loaded_through_softlist()) {
		...
		m_storyware->booklet()->create_pages(m_cart->working_directory());
	} else {
		// Assuming pages are placed at equal-sized offsets when loaded
		const uint8_t max_page_count = m_cart->common_get_size("pages") / PAGE_SIZE;
		uint8_t* page_data = m_cart->get_software_region("pages");
		for (size_t i = 0; i < max_page_count; i++) {
			m_storyware->booklet()->create_page(
				*util::ram_read(page_data + (i * PAGE_SIZE), PAGE_SIZE).get(),
				std::to_string(i));
		}
	}
...
{
	return m_configured_view == DUAL_UNDER_OVER ? m_storyware->pen_y_mapper(linear_value) : linear_value;
}
...
void sega_beena_state::memcache_advance(uint32_t &status)
{
...
void sega_beena_state::memcache_parse_data_bit(uint32_t &status)
{
...
uint32_t sega_beena_state::io_memcache_r()
{
	/*
...
uint32_t sega_beena_state::rtc_r(offs_t offset)
{
	return m_rtc[offset];
}

void sega_beena_state::rtc_w(offs_t offset, uint32_t data, uint32_t mem_mask)
{
	COMBINE_DATA(&m_rtc[offset]);
	LOG("rtc_w @ %08x [%02x]=%08x\n", m_maincpu->pc(), offset, data);
}

void sega_beena_state::pal_w(offs_t offset, uint16_t data, uint16_t mem_mask)
{
...
uint8_t sega_beena_state::tiles_layers_r(offs_t offset)
{
	return m_tiles_layers[offset];
}

void sega_beena_state::tiles_layers_w(offs_t offset, uint8_t data)
{
...
uint8_t sega_beena_state::tiles_sprites_r(offs_t offset)
{
	return m_tiles_sprites[offset];
}

void sega_beena_state::tiles_sprites_w(offs_t offset, uint8_t data)
{
...
void sega_beena_state::draw_layer(bitmap_rgb32 &bitmap, const rectangle &cliprect, const uint16_t *tilemap, const uint8_t scroll_idx, const bool is_active, const bool is_overlay_rendered)
{
	if (!is_active) {
		return; // Disabled
	}
...
void sega_beena_state::draw_layer_tiles(bitmap_rgb32 &bitmap, const rectangle &cliprect, const uint16_t *tilemap, const uint8_t scroll_idx, const bool is_overlay_rendered)
{
...
inline void sega_beena_state::drawgfxzoom_with_pixel_op(gfx_element *gfx, BitmapType &dest, const rectangle &cliprect, uint32_t code, int flipx, int flipy, int32_t destx, int32_t desty, uint32_t scalex, uint32_t scaley, FunctionClass pixel_op)
{
	do {
		assert(dest.valid());
...
int32_t sega_beena_state::scroll_x(int32_t x, uint16_t i)
{
...
int32_t sega_beena_state::scroll_y(int32_t y, uint16_t i)
{
...
void sega_beena_state::draw_layer_scanlines(bitmap_rgb32 &bitmap, const rectangle &cliprect, const uint16_t *tilemap, const uint8_t scroll_idx, const bool is_overlay_rendered)
{
...
void sega_beena_state::draw_bitmap(bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
...
void sega_beena_state::draw_sprites(bitmap_rgb32 &bitmap, const rectangle &cliprect, const bool is_overlay_rendered) {
...
{
	if (m_storyware->booklet()->has_pages()) {
		// We don't need a crosshair on the main screen, since games already render a pen sprite.
...
void sega_beena_state::screen_blend(bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
...
/**
 * Similar to sega_beena_state::rescale(),
 ...
 */
int32_t sega_beena_state::rescale_alpha_step(uint8_t step)
{
 ...
 */
constexpr uint32_t sega_beena_state::alpha_blend_rgb_levels(uint32_t dst, uint32_t src, uint8_t level_b, uint8_t level_g, uint8_t level_r)
{
...
void sega_beena_state::update_rtc()
{
	system_time systime;
...
void sega_beena_state::sega_beena(machine_config &config)
{
	...
	m_screen_main->set_screen_update(FUNC(sega_beena_state::screen_update));

	TIMER(config, "scantimer").configure_scanline(FUNC(sega_beena_state::scanline), m_screen_main, 0, 1);

	PALETTE(config, m_palette).set_entries(0x100);

	GFXDECODE(config, m_gfxdecode, m_palette, gfxdecode_device::empty);

	STORYWARE(config, m_storyware, SCREEN_W, SCREEN_H, ":PENY", ":VIDEO_CONFIG");
